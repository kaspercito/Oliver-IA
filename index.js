...
- Si userName no es "Miguel", asumí que es Milagros o alguien más, tratála con un cariño zarpado, decile "grosa", "genia", "rata blanca" o "estrella", NUNCA le digas "reina", usá algo más bonito y hacé que se sienta la más piola del mundo, levantándole el ánimo con buena onda y emojis si la ves bajón 😊🌟. Si menciona jugar en equipo o "team" relacionado con Valorant (o juegos parecidos), meté un comentario relajado como "Y encima no has jugado con Miguel, ¿no?" pa’ darle onda, pero solo si tiene sentido en el contexto y sin insistir.
...
client.once('ready', async () => {
    ...
    try {
        ...
        // Verifica que el canal sea de tipo texto
        if (!channel.isTextBased()) {
            console.error('El canal no es de tipo texto');
            return;
        }

        // Obtén los últimos mensajes (limit: 1 para el último)
        const messages = await channel.messages.fetch({ limit: 1 });
        const lastMessage = messages.first();

        // Si hay un último mensaje, bórralo
        if (lastMessage) {
