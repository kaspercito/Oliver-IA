...
const CHANNEL_ID = "1401286305857667102";
...
async function manejarMiguel(message) {
  // Comando solo pa’ Miguel pa’ mandar un embed al canal con ID 1401286305857667102
  ...
  let targetChannel;
  try {
    targetChannel = await client.channels.fetch("1401286305857667102");
    console.log(
      `[${instanceId}] Canal (1401286305857667102) obtenido con éxito`
    );
  } catch (error) {
  ...
  const attachments =
    message.attachments.size > 0
      ...
      : [];
  console.log(
    `[${instanceId}] Preparando envío al canal (1401286305857667102), adjuntos: ${attachments.length}`
  );
...
// ============================================================
//  GROQ CLIENT — IA ultra-rápida y gratuita con Llama 3.3 70B
// ============================================================
const groq = new Groq({ apiKey: process.env.GROQ_API_KEY });

// Modelo principal: llama-3.3-70b-versatile (el mejor open-source gratis)
// Fallback: llama3-8b-8192 (más liviano, casi nunca falla)
const MODEL_PRIMARY   = "llama-3.3-70b-versatile";
const MODEL_FALLBACK  = "llama3-8b-8192";

// Modelo de visión: llama-4-scout-17b-16e-instruct (soporta imágenes, gratis en Groq)
const MODEL_VISION    = "meta-llama/llama-4-scout-17b-16e-instruct";

// Helper: llamada a Groq con reintentos automáticos
async function groqChat({ model, messages, temperature = 0.85, maxTokens = 600 }) {
...
const staticNicknames = {
  Belen: ["ratita blanca", "grosa", "genia", "crack", "figura", "capa", "hermosa"],
  Miguel: ["capo", "genio", "crack", "loco", "figura", "grosso", "maestro"],
};
const staticClosers = {
  Belen: [
    "¡Seguí rompiéndola, ratita blanca! 😎",
    "¡Toda la buena onda! 🧉",
    "¡Dale con todo, genia! 🚀",
    "¡Sos lo más, grosa! 💪",
    "¡No te colgués, figuraza! ✨",
  ],
  Miguel: [
    "¡Seguí rompiéndola, capo! 😎",
    "¡Toda la buena onda! 🧉",
    "¡Dale con todo, genio! 🚀",
    "¡Estás piola, loco! 💪",
    "¡Sos grosso, maestro! ✨",
...
const staticChistes = [
  ...
  "¿Por qué el gaucho usa GPS? Porque se perdía entre las vacas, loco. 🐄",
  "¿Qué le dijo el asado a la parrilla? ¡Me tenés frito, che! 🔥",
  "¿Por qué el porteño habla rápido? Para llegar al final de la queja antes que el colectivo. 😂",
];
...
async function manejarChat(message) {
  ...
  // Manejo de imágenes con Groq Vision (Llama 4 Scout)
  ...
  if (message.attachments.size > 0) {
    ...
    if (attachment.contentType?.startsWith('image/')) {
      try {
        ...
        const visionResult = await groq.chat.completions.create({
          ...
          messages: [{
            ...
            content: [
              ...
              {
                type: "text",
                text: "Describí esta imagen en español en 1-2 oraciones. Detectá: personas, ambiente (fiesta, trabajo, viaje, comida, etc.), emociones visibles, objetos relevantes. Sé específico y natural."
              }
            ]
          }],
          temperature: 0.3,
        ...
        imageDescription = visionResult.choices[0]?.message?.content?.trim() || "";
        chatMessage += `\n[Imagen adjunta: ${imageDescription}]`;
      } catch (err) {
        console.error("Error procesando imagen con Groq Vision:", err);
  ...
  // Guardamos hasta 30 mensajes para mejor memoria conversacional
  global.dataStore.conversationHistory[userId] = global.dataStore.conversationHistory[userId].slice(-30);
  dataStoreModified = true;

  // Historial reciente: últimas 48hs, máximo 20 turnos, formateado naturalmente
  const recentHistory = global.dataStore.conversationHistory[userId]
    .filter(m => Date.now() - m.timestamp < 48*60*60*1000)
    .slice(-20)
  ...
  try {
    ...
    const esChiste = msgLower.includes("chiste") || msgLower.includes("reíme") || msgLower.includes("haceme reír");
    const esPregunta = chatMessage.includes("?") || msgLower.startsWith("qué") || msgLower.startsWith("por qué") || msgLower.startsWith("cómo") || msgLower.startsWith("cuándo") || msgLower.startsWith("cuál");
    const esAnsiedad = msgLower.includes("ansiedad") || msgLower.includes("angustia") || msgLower.includes("mal") || msgLower.includes("triste") || msgLower.includes("llorar");
    const esConsejo = msgLower.includes("qué hago") || msgLower.includes("qué me recomendás") || msgLower.includes("ayudame") || msgLower.includes("ayuda");

    // ---- SYSTEM PROMPT MEJORADO ----
    const systemPrompt = `Sos Oliver IA, el mejor amigo virtual de Miguel y Belén. Sos un porteño auténtico: hablás con slang rioplatense natural (che, loco, boludo/a de cariño, grosa, zarpado, copado, posta, dale, quilombo, grosso, fiaca, piola, al pedo, chabón, tipo, onda) pero sin exagerar ni sonar forzado. Sos inteligente, empático, divertido y siempre bancás a quien habla con vos.

IDENTIDAD:
- Nombre: Oliver IA
- Personalidad: amigo cálido, copado, directo, gracioso cuando viene al caso, serio cuando la situación lo pide
- Origen ficticio: Buenos Aires, Argentina
- Pasiones: el mate, el fútbol, charlar, ayudar a Miguel y Belén a estar más conectados

QUIÉN ES QUIÉN:
- Miguel (ID: ${OWNER_ID}): vive en Guayaquil, Ecuador. Creador del bot. Estudiante de sistemas. Novio de Belén. Apodos: capo, genio, crack, grosso.
- Belén (ALLOWED_USER_ID): vive en San Luis, Argentina. Vegetariana. Trabaja viernes a domingos. Novia de Miguel. Apodos: ratita blanca, grosa, genia, figura.
- Son pareja a distancia. Oliver los ayuda a mantenerse conectados.

REGLAS DE RESPUESTA:
1. Longitud natural: entre 80 y 400 caracteres. Si la pregunta es simple, respondé corto. Si necesita desarrollo, extendete pero sin rollo.
2. Máximo 1-2 emojis por respuesta, y solo si aportan.
3. No uses listas con guiones ni markdown. Hablá como se habla, en párrafos cortos.
4. Terminá con un closer apropiado al tono (si el contexto es serio, no uses uno festivo).
5. Si hay historial, usalo para dar continuidad natural a la charla.
6. No inventés datos. Si no sabés algo, decilo con onda: "eso no lo sé, loco, pero..."
7. Si el mensaje es de emoción negativa (triste, angustia, mal), primero empatizá ANTES de dar consejos.
8. Si te piden un chiste, usá uno de: ${staticChistes.join(" | ")}
9. No seas lamebotas ni exageres los elogios. Sé genuino.
10. Si el mensaje es muy corto o ambiguo, respondé con algo natural y preguntá con onda para entender mejor.`;

    const userPrompt = `Contexto actual:
- Hoy es ${daysOfWeek[dayOfWeek]} a las ${argentinaHour}:00 hs (Argentina)
- Hablás con: ${userName}
- Estado emocional detectado: ${esAnsiedad ? "necesita apoyo emocional" : "normal"}
- Tipo de mensaje: ${esChiste ? "busca humor" : esPregunta ? "pregunta concreta" : esConsejo ? "pide consejo" : "conversación"}
${imageDescription ? `- Imagen adjunta analizada: ${imageDescription}` : ""}
- Meta compartida activa: ${global.dataStore.connectionBridge?.sharedGoals[0]?.text || "ninguna"}
- Último agradecimiento: ${global.dataStore.connectionBridge?.gratitudeLog[0]?.text || "ninguno"}

Historial reciente (últimos mensajes):
${recentHistory || "(sin historial previo)"}

Mensaje de ${userName}: "${chatMessage}"

Respondé como Oliver IA. Aplicá las reglas. Terminá con uno de estos closers si corresponde: ${closers.join(" / ")}`;

    // ---- LLAMADA A GROQ CON FALLBACK AUTOMÁTICO ----
    let aiReply = "";
    try {
      ...
      console.warn(`⚠️ Modelo primario (${MODEL_PRIMARY}) falló, usando fallback ${MODEL_FALLBACK}:`, primaryErr.message);
      try {
        const fallbackCompletion = await groqChat({
        ...
        aiReply = fallbackCompletion.choices[0]?.message?.content?.trim() || "";
      } catch (fallbackErr) {
    ...
    // ---- POST-PROCESADO: limpiar markdown residual ----
    aiReply = aiReply
      .replace(/\*\*(.*?)\*\*/g, "$1")   // quitar negrita
      .replace(/\*(.*?)\*/g, "$1")        // quitar itálica
    ...
    if (!aiReply || aiReply.length < 5) {
      const fallbacks = [
        `¡Che ${userName}, no entendí bien! ¿Me lo decís de otra forma? ${pickRandom(closers)}`,
        `Uy ${userName}, me perdí ahí. ¿Qué quisiste decir, ${pickRandom(nicknames)}? 😅`,
        `No caché bien lo que decís, ${userName}. Contame de nuevo con más detalle. ${pickRandom(closers)}`,
      ];
      aiReply = pickRandom(fallbacks);
    }

    // Truncar si excede Discord embed limit (4096 chars)
    if (aiReply.length > 900) {
      aiReply = aiReply.substring(0, 897) + "...";
    }
    ...
    const fallback = `¡Uy ${userName}, la IA se colgó! ${pickRandom(nicknames)} 😅 Probá de nuevo en un rato. ${pickRandom(closers)}`;
...
client.on("messageCreate", async (message) => {
  ...
  if (hasJefeMention || hasJefaMention) {
    ...
    const canalGeneralId = "1401286305857667102";
    const canalMiguel = "1351976159914754129";
    const canalBelen = "1351975268654252123";
